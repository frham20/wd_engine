...
namespace wd
{
	...
	bool window_platform::init( const char* title, const recti& region )
	{
		...
		auto result =
		    ::CreateWindowEx( exstyle, window_class_name, title, style, region.left, region.top, region.width(),
		                      region.height(), nullptr, nullptr, engine_platform::get_hinstance(), this );
		if ( result != m_hwnd )
			return false;
